...
vacayLatLong <- data.frame(lat = c(vacayData[,"lat"]), lng= c(vacayData[,"lng"]))
...
vacayLatLong %>% leaflet() %>% addTiles() %>% addMarkers(~lng, ~lat, icon = vacayIcons, popup = vacaySites$Link_URL, clusterOptions = markerClusterOptions(zoomToBoundsOnClick = TRUE))
```
